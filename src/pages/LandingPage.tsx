      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md h-20 shadow-md border-b border-slate-100' : 'bg-transparent h-24'}`}>
        <div className="container mx-auto px-10 h-full flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            <Link to="/login" className="text-slate-900 font-heading font-black text-[11px] uppercase tracking-[0.2em] hover:text-blue-600 transition-colors">Entrar</Link>
            <Button size="md" onClick={() => navigate('/register')} className="shadow-2xl shadow-blue-600/30">Teste Grátis</Button>
          </div>
        ...
        <div className="container mx-auto px-10 flex flex-col lg:flex-row items-center gap-20">
          <div className="lg:w-1/2 space-y-10 text-center lg:text-left">
            <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-widest">
              <Star size={14} className="fill-blue-600" /> CONFORMIDADE NR-01 & NR-17
            </div>
            <h1 className="text-6xl lg:text-[80px] font-heading font-black text-slate-900 leading-[0.95] tracking-tight">
              Gestão de Riscos <br/><span className="text-blue-600">Psicossociais.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-5 justify-center lg:justify-start pt-4">
              <Button size="lg" onClick={() => navigate('/register')} className="h-20 px-12 text-base shadow-2xl shadow-blue-600/20">
                COMEÇAR AGORA <ArrowRight className="ml-3 w-6 h-6" />
              </Button>
          ...
          <div className="lg:w-1/2 w-full">
            <div className="bg-white rounded-[48px] shadow-2xl border-[16px] border-white overflow-hidden transform rotate-2 hover:rotate-0 transition-transform duration-700">
               <div className="bg-slate-100 p-4 border-b border-slate-200 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <div className="flex gap-2"><div className="w-3 h-3 rounded-full bg-red-400/20"></div><div className="w-3 h-3 rounded-full bg-amber-400/20"></div><div className="w-3 h-3 rounded-full bg-emerald-400/20"></div></div>
                  app.nrzen.com.br
               </div>
               <div className="p-12 space-y-10">
                  ...
                  <div className="grid grid-cols-2 gap-8">
                     <div className="bg-blue-50 rounded-[32px] p-8 border border-blue-100"><span className="text-[10px] font-black text-blue-600 uppercase tracking-widest block mb-2">Empresas</span><span className="text-5xl font-black text-blue-900 tracking-tighter">24</span></div>
                     <div className="bg-emerald-50 rounded-[32px] p-8 border border-emerald-100"><span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest block mb-2">Monitorados</span><span className="text-5xl font-black text-emerald-900 tracking-tighter">892</span></div>
                  </div>
               </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-10 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
            ...
            <div className="flex items-center justify-center gap-8 mt-12">
              <span className={`text-[12px] font-black uppercase tracking-widest transition-colors ${billingCycle === 'monthly' ? 'text-white' : 'text-slate-600'}`}>Mensal</span>
              <button onClick={() => setBillingCycle(prev => prev === 'monthly' ? 'yearly' : 'monthly')} className="w-20 h-10 bg-slate-900 rounded-full p-1 relative border-2 border-slate-800">
              ...
              <div className="flex flex-col items-start">
                <span className={`text-[12px] font-black uppercase tracking-widest transition-colors ${billingCycle === 'yearly' ? 'text-white' : 'text-slate-600'}`}>Anual</span>
                <span className="bg-emerald-500 text-white text-[9px] uppercase font-black px-2 py-0.5 rounded mt-1 animate-pulse tracking-tighter">Economize 20%</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-stretch max-w-7xl mx-auto">{PLANS.map(plan => renderPriceCard(plan))}</div>
        </div>
      </section>
