    ...
    <Layout>
      <header className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Gerencie sua conta e upgrade de recursos.</p>
        </div>
        ...
        <div className="bg-slate-100 p-1.5 rounded-2xl flex items-center shadow-inner">
          ...
          >
            Anual <span className="bg-emerald-100 text-emerald-600 text-[9px] px-1.5 py-0.5 rounded-md">-17% OFF</span>
          </button>
        </div>
      </header>
      ...
      <Card className="mb-10 bg-slate-900 border-none relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 p-2">
          <div className="text-white text-center md:text-left">
            ...
            <p className="text-slate-400 mt-1 font-medium italic">Sua consultoria está habilitada no sistema.</p>
          </div>
          <div className="flex gap-3">
             ...
             >
                ...
                Gestão de Faturas
                {!loadingPortal && <ExternalLink size={14} className="ml-2 opacity-40" />}
             </Button>
          </div>
        </div>
      </Card>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto pb-12">
        {selfServicePlans.map((plan) => {
          ...
          return (
            <Card 
              key={plan.id} 
              className={`relative flex flex-col h-full transition-all duration-300 hover:shadow-2xl ${plan.popular ? 'border-blue-500 shadow-xl scale-[1.03] ring-4 ring-blue-50' : 'border-slate-200'}`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">
                  Recomendado
                </div>
              )}
              ...
              <Button 
                ...
                className="h-16 uppercase text-xs font-black tracking-[0.2em]"
              >
                {isLoading ? (
      ...
      <div className="mt-8 text-center">
         <div className="inline-flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] opacity-60">
            <Zap size={14} className="text-amber-500" />
            Infraestrutura de Pagamento Stripe SSL
         </div>
      </div>
    </Layout>
  );
